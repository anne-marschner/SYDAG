...
public class SchemaNoise extends Noise {
    ...
    private List<String> allSelectedMethods = new ArrayList<>();   // List of methods chosen by th user

    private List<String> availableMethods = new ArrayList<>();     // List of methods that have been chosen but not applied yet


    /**
     * Constructs a SchemaNoise object with the specified list of selected noise methods.
     *
     * @param allSelectedMethods a list of method names selected by the user for perturbing the schema.
     */
     ...
     *
     * @param relation the Relation whose schema will be perturbed.
     ...
     * @return a Relation object with the perturbed schema.
    ...
    public Relation perturbSchema(Relation relation, Integer noisePercentage, boolean schemaNoiseInKeys, boolean deleteSchema) throws Exception {
        ...
        if (relation.getNumOfOverlappingRows() != null) {
     ...
     *
     * @param relation the Relation which headers should be removed.
     * @return the Relation without headers.
     */
    ...
    /**
     * Selects a noise method to perturb the given column name.
     *
    ...
    public String chooseNoise(String columnName) throws Exception {
        ...
        if (availableMethods.contains("shuffleWords")) {
            if (containsMultipleWords) {
                ...
                availableMethods.remove("shuffleWords"); // Will be added again after random choice of another method
                removedShuffleMethod = true; // Set to true to remember that the method needs to be added again
            }
        }
        ...
        if (availableMethods.contains("removeVowels")) {
            ...
            availableMethods.remove("removeVowels"); // Will be added again after random choice of another method
            removedVowelMethod = true; // Set to true remember that method needs to be added again
        }
        ...
        // Try other already used methods
    ...
    private String findReplacementFromAvailableMethods(String columnName, boolean removedShuffleMethod, boolean removedVowelMethod) throws Exception {
        ...
        for (String selectedMethod : availableMethods) {
            ...
            if (!replacement.equals(columnName) && !replacement.equals(" ") && !replacement.equals("")) {
                ...
                // Remove used method so that it will not be used again before all selected methods have been used
                availableMethods.remove(selectedMethod);

                // Add the methods that were removed because they could not be applied to the given columnName
                if (removedShuffleMethod) {
                    availableMethods.add("shuffleWords");
                }
                if (removedVowelMethod) {
    ...
    private String useFallbackMethod(String columnName, boolean removedShuffleMethod, boolean removedVowelMethod) throws Exception {
        ...
        // Select a fallback method that guarantees change
